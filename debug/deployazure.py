...
for i, size in enumerate(vmlist):
  ...
  servers.append(info)
  run(f'ssh -o StrictHostKeyChecking=no -i ./keys/id_rsa ansible@{ip} "mkdir ~/WorkflowSchedulingCwk"')
  run(f'rsync -Pav -e "ssh -i ./keys/id_rsa"  ~/WorkflowSchedulingCwk/functions/ ansible@{ip}:/home/ansible/WorkflowSchedulingCwk/functions/')
